...
async function generateAIResponse(query: string, matchedListings: any[], externalResults: any[]) {
  ...
  const systemPrompt = `You are an AI assistant for a AI detection tools for text, images, and videos directory.

**Your scope:**
- ONLY answer questions about: AI detection, content verification, plagiarism check
- Recommend tools from our database
- Provide comparisons and reviews
- Cite external sources with [1], [2]

**Off-topic handling:**
If the query is NOT about AI detection, content verification, plagiarism check, respond:
"I specialize in AI detection, content verification, plagiarism check. For questions about [detected topic], please use a relevant search engine or directory."
